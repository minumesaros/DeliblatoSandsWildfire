// CiROCCO FireRiskApp V008

/**** ASSETS ****/
var roi = ee.FeatureCollection("projects/ee-minucher/assets/DeliblatoROI_SQFIN");
var roisrp = ee.FeatureCollection("projects/ee-minucher/assets/SRPDeliblatoWGS84");
var stat4 = ee.Image("projects/ee-minucher/assets/StaticV4");
...
/**** LIMITS  ****/
var PAST_MIN_YEAR = 2020;
var PAST_MIN_MONTH = 1;
var PAST_MIN_DAY = 1;

var PAST_MAX_YEAR = 2026;
var PAST_MAX_MONTH = 3;
var PAST_MAX_DAY = 31;

var PAST_MIN_DATE = new Date(Date.UTC(PAST_MIN_YEAR, PAST_MIN_MONTH - 1, PAST_MIN_DAY));
var PAST_MAX_DATE = new Date(Date.UTC(PAST_MAX_YEAR, PAST_MAX_MONTH - 1, PAST_MAX_DAY));
...
function makeForecastDateList() {
  var dates = [];
  var now = new Date();
  var todayUTC = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));

  for (var i = 0; i <= 15; i++) {
    var d = new Date(todayUTC);
    d.setUTCDate(todayUTC.getUTCDate() + i);
    dates.push(dateToStringUTC(d));
  }

  return dates;
}

var forecastDates = makeForecastDateList();
...
function buildForecastRiskResult(dateString) {
  ...
  return {
    fireRisk: fireRisk,
    meteo: meteo,
    runLabel: ee.Date(latestForecastCreation).format('YYYY-MM-dd HH:mm')
  };
}
...
function setPastDate(jsDate) {
  if (jsDate < PAST_MIN_DATE) jsDate = new Date(PAST_MIN_DATE);
  if (jsDate > PAST_MAX_DATE) jsDate = new Date(PAST_MAX_DATE);
...
function updateModeUI() {
  ...
  meteoLabel.setValue('');
  statusLabel.setValue('Ready. Press Show map.');
}
...
function renderCurrentSelection() {
  ...
  if (mode === 'Short term Forecast (15 days)') {
    ...
    statusText = 'Short term Forecast (15 days): ' + fcDate + ' at 13:00 UTC';

    forecastResult.runLabel.evaluate(function(runLabel) {
      if (timeframeSelect.getValue() !== 'Short term Forecast (15 days)') return;
      statusLabel.setValue(statusText + ' | latest run: ' + runLabel + ' UTC');
    });
  ...
  if (mode === 'Long term climate (2031-2041)') {
    ...
    statusText = 'Long term climate (2031-2041): ' + climateScenario +
      ' | month ' + climateMonthSelect.getValue() +
      ' | 2031–2041 ensemble median';
  }
...
// Faster than centerObject(), because it does not need to query geometry bounds at startup.
map.setCenter(21.05, 44.90, 10);
...
pastYearSelect.onChange(function() {
  refreshPastMonthOptions();
});

pastMonthSelect.onChange(function() {
  refreshPastDayOptions();
});
...
var forecastInfo = ui.Label('Forecast dates generated locally: today + 15 days', {
  color: 'gray',
  margin: '4px 0 0 0',
  fontSize: '11px'
});
...
var statusLabel = ui.Label('Ready. Press Show map.', {
  color: 'gray',
  margin: '8px 0 0 0'
});
...
// Fast startup: set UI only, do not render risk layer automatically.
setPastDate(new Date(Date.UTC(PAST_MAX_YEAR, 7, 1)));  // default: 2025-08-01
updateModeUI();
